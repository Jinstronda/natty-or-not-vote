...
// Helper function to detect auto-generated descriptions that should be hidden from users
const isAutoGeneratedDescription = (description: string | null | undefined): boolean => {
  ...
  return autoGeneratedPrefixes.some(prefix => 
    description.toLowerCase().includes(prefix.toLowerCase())
  );
};
...
const InfluencerPhotoGallery: React.FC<InfluencerPhotoGalleryProps> = ({ photos, className = '', style }) => {
  ...
  // Only show description if it's not auto-generated
  const shouldShowDescription = current.description && !isAutoGeneratedDescription(current.description);
  ...
  return (
    <div className={`relative w-full aspect-square rounded-xl overflow-hidden bg-secondary flex items-center justify-center ${className}`} style={style}>
      ...
      />
      {/* Overlay description - only show if not auto-generated */}
